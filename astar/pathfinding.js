function createMatrix () {
    ...
    for (let i = 0; i < size; i++){
        ...
        for (let j = 0; j < size; j++){
            matrix[i][j] = {f: Infinity, h: Infinity, parent: null};
        }
    }
    return matrix;
}

function searchOptimalPoint(matrix, queue){
...
function elementInQueue(queue, value) {
    let result = false;
    for (let i = 0; i < queue.length; i++){
        if (value === queue[i]){
            result = true;
...
function distanceOfCityBlocks (start, finish){ //сумма модулей разностей координат
    let s1 = Math.abs(parseInt(finish.getAttribute("pos")/size) - parseInt(start.getAttribute("pos")/size));
    let s2 = Math.abs((finish.getAttribute("pos")%size) - (start.getAttribute("pos")%size));
    let sum = s1 + s2;
    return sum;
}

function findNeighboursOfPoint(point){
...
document.querySelector("#findPathButton").onclick = async function(){
    ...
    let startingPosition = startPoint.getAttribute("pos");
    
    matrix[parseInt(startingPosition/size)][startingPosition%size].f = distanceOfCityBlocks(startPoint, finishPoint);
    matrix[parseInt(startingPosition/size)][startingPosition%size].h = 0;
    ...
    while (queue.length > 0){
        ...
        if (currMatrixPoint === finishPoint){
            ...
            let matrixPoint = matrix[parseInt(bestIndex/size)][bestIndex%size].parent.getAttribute("pos");
            while (matrix[parseInt(matrixPoint/size)][matrixPoint%size].parent !== null){
                ...
                await sleep(10);
                matrixPoint = matrix[parseInt(matrixPoint/size)][matrixPoint%size].parent.getAttribute("pos");
            }
        ...
        for (let i = 0; i < currNeighbours.length; i++){
            let neighbourInd = currNeighbours[i].getAttribute("pos");
            let temporaryAssessment = matrix[parseInt(bestIndex/size)][bestIndex%size].h + 1;
            if (temporaryAssessment < matrix[parseInt(neighbourInd/size)][neighbourInd%size].h){ //временная оценка < стоимомти пути до соседней точки
                matrix[parseInt(neighbourInd/size)][neighbourInd%size].parent = currMatrixPoint;
                matrix[parseInt(neighbourInd/size)][neighbourInd%size].h = temporaryAssessment;
                matrix[parseInt(neighbourInd/size)][neighbourInd%size].f = temporaryAssessment + distanceOfCityBlocks(currNeighbours[i], finishPoint);
