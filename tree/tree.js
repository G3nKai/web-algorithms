...
function decideToSplit(matrix) {
    ...
    function doubleData(columnIngex) {
        ...
        for (let i = 0; i < nums.length; ++i) {
            if (nums[i].value < result["splittingParameter"])  result["indexes"]["leftIndexes"].push(i);
            else result["indexes"]["rightIndexes"].push(i);
        }
    ...
    function doubleEntropy (nums) {
        ...
        for (let val of celSet) cels[val] = 0;

        for (let i = 0; i < nums.length; ++ i) cels[nums[i].celElem] += 1;
        

        let localEntropy = 0;

        for (let val of celSet) localEntropy -= (cels[val] / nums.length) * getBaseLog(2, cels[val] / nums.length);
    ...
    function decisionValue() {
        ...
        for (let val of celSet) celDict[val] = 0;

        for (let i = 1; i < matrix.length; i++) celDict[matrix[i][matrix[0].length - 1]]++;
    ...
    function isDouble(columnIngex) {
        for (let i = 1; i < matrix.length; ++ i) 
            if (isNaN(parseFloat(matrix[i][columnIngex]))) return false;
